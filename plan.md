...
- **The characteristic criterion.** A world detail earns its place
  ...
  for the DM — the trait sketch is the type case.
...
### Ready to build — two short specs

Either fits in any session; neither depends on the other or on the
design session below.

**Spec A — the high-level test start.** `new --level N` (2-20; absent
= the ordinary L1 start, unchanged) starts a fresh save with the party
at level N, everything autogenerated — playtesting for the bands no
campaign reaches (the played-reality note: nothing above L4 has ever
been played, and conquest has never been played at all).

- PC and the long-time companion generated at level N
  (`people.make_character` takes any level); BOTH autospend
  (`rpg.autospend_points`) — the levelup menu is for played
  progression, not for paging through 30 points by hand. Print a
  compact summary of what was bought. Optional `--race R` fixes the
  PC's race.
- Gear and purse: ordinary starting-weapon rolls, then the PC claims
  `weapons.reward_weapon_for_level(N)`; purse at the implementer's
  judgement (a modest fraction of career earnings to N — this is
  testing convenience, not economy fidelity); kit at the standard
  KIT_* lines.
- The pact: initialize the pin ledger (`last_pin_served`) to the
  highest pin at or below N, so a high start does not open with a
  backlog of hell assignments; the next odd level pins normally.
  `--no-pact` composes as usual.
- Story waves gate on party level and will fire early on a high-N
  settlement stop — intended; that is part of what gets tested.
- Session-side only (`cmd_new`); the sims and benches never see it.
  Smoke: `new --level 8 --seed 1`, then `status`, `board`, one fight.

**Spec B — the NPC trait rollback.** Dict NPCs stop carrying the
three-trait sketch; heroes keep it. The criterion is the reason:
companion traits are chosen against (hiring) and several are
mechanical (armored, wealthy, interests); giver and notable traits
are unbacked flavor. What dict NPCs carry INSTEAD (a want, a problem,
a disposition) is the design session's question — this rollback just
stops paying for flavor nothing reads.

- `people.make_npc` drops the `roll_traits` call and the `traits`
  key, keeping race/sex/age/name/role (and `level` where passed);
  `people.npc_line` drops the trait bits.
- Delete the giver/recipient trait dumps on the quest readouts
  (`quests.py`'s show path and demo).
- Callers that need nothing beyond the vanished flavor: givers,
  notables, service keepers, delivery recipients, house residents,
  posse and hell leaders, conquest defenders, war faces, smiths.
- The hero path (`make_character`, candidate sheets, `hero_line`,
  `chatter`) is untouched.
- RNG note: `make_npc` consumes fewer rolls, so a seeded world casts
  different names downstream. Acceptable (no backwards
  compatibility); re-run the touched suites and fix seed-pinned
  expectations (`test_places`' house residents are the likely spot).
...
2. **The design session.** What it must settle, in order:
   ...
   - **What dict NPCs carry** in place of the trait sketch (spec B).
